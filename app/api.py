...
def fail(error_code, error_message):
    return {'response': 'Fail', 'error_code':error_code, 'error_message':error_message}
...
def remove_tag_from_image(image_id, tag_name):
    ...
    if tag not in image.tags:
        return fail(405, f'<Image {image_id} not tagged with "{tag_name}" - cannot remove')
...
def get_thumbnail_image(image_id):
    ...
    if not image:
        return fail(404, f"<Image {image_id}> not found")
    image_data = PImage.open(current_app.config['IMAGE_PATH'] +'thumbnails\\'+ image.filename)
    image_io = BytesIO()
    image_data.save(image_io, 'png')
    image_io.seek(0)
...
def get_images_by_best_tag_match():
    ...
    q = (select(image_tags.c.image_id, func.count(image_tags.c.image_id).label('match'), func.aggregate_strings(image_tags.c.tag_id,',').label('tags'))
    ...
    return build_success({
        ...
        'images': [{'image_id': o[0], 
                    'matches':o[1], 
                    'tags': [int(i) for i in o[2].split(',')]} for o in db.session.execute(q).all()]
        })
