...
@click.option('-r', '--regenerate-thumbnails', is_flag=True, default=False, help="Force regeneration of thumbnails")
@click.option('-z', '--thumbnail-zoom', default=0.25, help='Zoom factor used to scale for thumbnails. Has no effect if not regenerating thumbnails')
@click.option('-f', '--force_reparse', is_flag=True, default=False, help="Delete the database and rescan the folder")
def parse_images(silent, regenerate_thumbnails, thumbnail_zoom, force_reparse):
    ...
    for image_path in images:
        ...
        if exists is None:
            ...
            image = Image(filename=filename, dimension_x=x, dimension_y=y, hash=hex(hash))
            db.session.add(image)
            sprint(silent, f"[parse-image] Creating database entry for {filename}.")
        else:
            sprint(silent, f"[parse-image] Found database entry for {filename}, skipping")
    db.session.commit()

@cli.cli.command('load-messages')
@click.option('-m', '--message-file', help="The file containing messages. One per line.")
@click.option('-x', '--truncate-left', default=0, help="Remove the first 'x' characters from each line (e.g, for a bulleted list)")
@click.option('-r', '--clear', is_flag=True, default=False, help="Remove all previous entries and load from a file afresh")
@click.option('-s', '--silent', is_flag=True, default=False, help="Supress all output")
