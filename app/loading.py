...
def loading_page():
    ...
    focus = Focus(image)
    return render_template('loading.html', imdata=imdata, message=message, focus=focus, title="Loading")
...
def backdrop_page():
    ...
    imdata = image.get_fullsize_image_data_base64(current_app.config['IMAGE_PATH'])
    focus = Focus(image)
    return render_template('backdrop.html', imdata=imdata, focus=focus, title="Backdrop")
